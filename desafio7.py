...
for elemento in tabla_json["results"]:
    ...
    #df["rowid"]=df.index
  #ordenando por availableQuantity
df=df.sort_values(by=['availableQuantity'],ascending=False)

#crear el campo stockPercentage

suma=sum(df['availableQuantity'])
division=(df['availableQuantity']/suma)*100
df=df.assign(stockPercentage=division.values)